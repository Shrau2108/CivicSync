...
import { fetchTasksForVolunteer, fetchTasks } from '@/services/api';
...
export function VolunteerTasksPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const t = profile?.role === 'admin' ? await fetchTasks() : await fetchTasksForVolunteer(user.id);
      setTasks(t);
    } catch (err) {
      ...
      {filtered.length === 0 ? (
        <Card><CardBody>
          <EmptyState
            ...
            description={tasks.length === 0 ? 'Browse available tasks to accept one' : 'Try a different search'}
          />
        </CardBody></Card>
      ) : (
